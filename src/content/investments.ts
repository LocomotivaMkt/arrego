 ...
 *
 * ÍCONE, NÃO EMOJI. `icon` guarda um PAPEL do catálogo fechado de `@/ui/Icon`,
 * nunca um glifo solto: emoji é ilustração que cada sistema desenha do seu
 * jeito e ignora a tinta do tema. O import é `import type`, então nada de
 * `@/ui` sobra em runtime aqui: conteúdo não passa a depender de componente.
 *
 * SEM TRAVESSÃO no texto que a pessoa lê. Este é o único arquivo do app com
 * parágrafo corrido de verdade, e o travessão é justamente a pontuação que faz
 * a frase longa parecer que nunca termina. Ponto, dois-pontos e parênteses dão
 * o mesmo respiro e ainda param a frase. A regra vale para as STRINGS; nos
 * comentários o traço pode ficar, porque comentário é código, não interface.
 *
 * Aqui a precisão vem antes da piada: esta é a parte do app onde uma frase mal
 * colocada custa dinheiro de verdade pra alguém. E é a parte onde o jargão
 * custa mais caro: quem já sabe o que é "principal", "contemplado" ou "CVM"
 * não precisa deste arquivo. Toda sigla se apresenta antes de ser usada.
 */
...
export type Topic = {
  id: string;
  /**
   * Papel do catálogo de `@/ui/Icon`, o assunto do tópico em um glifo.
   *
   * NENHUMA TELA DESENHA ISTO HOJE. A lista de /aprender usa o ícone da
   * SEGURANÇA (`SAFETY[topic.safety].icon`), de propósito: o que a pessoa
   * precisa distinguir de relance é o risco, não o assunto, que o título já
   * diz. O campo existe porque o assunto é dado do tópico e sobreviveu à troca
   * de emoji para ícone. Se continuar sem tela que o use, ele deve morrer.
   */
  icon: IconName;
  title: string;
...
export const TOPICS: readonly Topic[] = [
  {
    ...
    body: [
      ...
      'O tamanho dela depende de quanto custa o SEU mês, não de quanto você ganha. Some tudo que sai todo mês por obrigação: aluguel, contas, mercado, transporte. Esse total é o custo de um mês da sua vida. A reserva é esse custo multiplicado pelo número de meses que você quer conseguir atravessar sem entrar renda nenhuma. Quem tem salário fixo e estável costuma mirar em uns três meses. Quem é autônomo, freela ou tem renda que balança mira mais alto, porque o mês ruim chega sem avisar.',
      ...
      'Existe uma ordem chata e verdadeira: dívida cara primeiro, reserva depois, o resto por último. Dívida cara quer dizer o rotativo do cartão, que é o que sobra correndo juros quando você paga só o mínimo da fatura, e o cheque especial. Não existe aplicação segura no Brasil que renda mais rápido do que essas duas crescem. Guardar e dever ao mesmo tempo é encher um balde furado.',
    ...
    watchOut: [
      'Deixar a reserva presa em algo com carência, ou seja, com um prazo mínimo antes de poder sacar. A emergência não espera o vencimento.',
...

  {
    ...
    body: [
      ...
      'O Tesouro Selic acompanha a taxa básica de juros do país: taxa sobe, ele rende mais; taxa cai, ele rende menos. Mas o detalhe que importa é outro. O preço dele sobe um pouquinho a cada dia útil e nunca faz curva pra baixo. É o único título público em que vender antes do vencimento não costuma virar prejuízo.',
      'O Tesouro recompra o título todo dia útil, então o dinheiro volta rápido. Existe uma taxa de custódia cobrada pela B3, que é a bolsa de valores brasileira, calculada sobre o valor guardado. Valores menores costumam ser isentos dela. Essa regra muda de tempos em tempos: confira na página oficial do Tesouro Direto, não neste texto.',
...

  {
    ...
    body: [
      'CDB quer dizer Certificado de Depósito Bancário. Tira o nome difícil e sobra isto: o banco pega seu dinheiro emprestado, empresta pra outra pessoa mais caro, e te devolve com juros. O banco não paga por bondade. Pra ele o seu dinheiro é matéria-prima, e os juros que ele te paga são o preço dela.',
      ...
      'A maioria dos CDBs rende um percentual do CDI, que é uma taxa que anda coladinha na taxa básica de juros. "100% do CDI" quer dizer que o banco te entrega tudo o que essa taxa rendeu no período. "90% do CDI" entrega nove décimos disso. Repare que é uma comparação, não um valor fixo: se a taxa básica mudar amanhã, o "100% do CDI" continua valendo, só que sobre o número novo.',
      ...
      'Imposto de renda igual ao da renda fixa em geral: só sobre o rendimento, pela tabela regressiva, ou seja, 22,5% até 180 dias, 20% de 181 a 360, 17,5% de 361 a 720 e 15% acima de 720 dias. O banco desconta sozinho no resgate; você não precisa fazer conta nenhuma.',
    ],
    goodFor: [
...

  {
    ...
    icon: 'money',
    title: 'Poupança',
    oneLiner: 'A mais fácil de todas, e é a facilidade que você paga em rendimento.',
    safety: 'seguro',
    protectedBy: 'FGC até R$ 250 mil por CPF e por instituição',
    ...
    body: [
      ...
      'E cada depósito tem o próprio aniversário. Se você deposita todo mês em dias diferentes, tem vários aniversários rodando ao mesmo tempo dentro da mesma poupança. Um saque pode zerar o mês de um pedaço do dinheiro sem você perceber.',
...

  {
    ...
    icon: 'lock',
    title: 'LCI e LCA',
    oneLiner: 'Empréstimo pro banco sem imposto de renda, desde que você não precise mexer.',
    safety: 'seguro',
    protectedBy: 'FGC até R$ 250 mil por CPF e por instituição',
    liquidity: 'Só depois da carência, e boa parte só devolve mesmo no vencimento.',
    body: [
      'LCI é Letra de Crédito Imobiliário e LCA é Letra de Crédito do Agronegócio. Funcionam como um CDB: você empresta pro banco e ele te paga juros. A diferença é o destino: esse dinheiro financia imóveis ou o agronegócio, e o governo quis incentivar esses dois setores.',
      ...
      'Só que a isenção não é presente. Ela é o preço de outra coisa. Toda LCI e toda LCA tem carência: um período mínimo em que o dinheiro não sai de jeito nenhum. Não tem multa, não tem "pago pra sair antes", simplesmente não dá. E muitas só devolvem no vencimento, que pode ser daqui a anos.',
      ...
      'A garantia é a mesma dos CDBs: FGC, R$ 250 mil por CPF e por instituição, contando o que você aplicou mais os juros que já correram, dentro do teto global de R$ 1 milhão renovável a cada 4 anos.',
    ],
    goodFor: [
...

  {
    ...
    body: [
      ...
      'Agora a parte que derruba gente todo ano: marcação a mercado. Traduzindo: o preço do seu título muda todo dia, porque enquanto ele existe tem gente comprando e vendendo ele por aí. Se hoje passarem a oferecer títulos novos pagando mais que o seu, ninguém quer o seu pelo mesmo preço, e ele fica mais barato pra compensar. Se os novos pagarem menos, o seu fica mais valioso.',
      ...
      'Daí sai a regra inteira: segurou até o vencimento, recebe exatamente o combinado, que é a inflação do período mais a taxa, sem susto. Vendeu antes, recebe o preço do dia, que pode ser mais e pode ser bem menos do que você pagou. Não é golpe, não é bug, não é o Tesouro te devendo. É o mecanismo funcionando como foi desenhado.',
      'Imposto: a tabela regressiva normal da renda fixa, só sobre o rendimento, ou seja, 22,5% até 180 dias, 20% de 181 a 360, 17,5% de 361 a 720 e 15% acima de 720 dias. Como o vencimento costuma ser longo, quem segura até o fim cai na menor faixa.',
    ],
    goodFor: [
...

  {
    ...
    body: [
      'Nada aqui é proibido nem pecado. O problema é o nome na etiqueta: são coisas vendidas como "investimento" que não se comportam como investimento. Investimento tem regra conhecida e risco que dá pra medir. O que está nesta lista tem resultado que depende de sorte, de outra pessoa, ou de uma promessa que ninguém assinou.',
      ...
      '"Robô de trades", ou "robô que rende sozinho": alguém promete um programa que opera por você e entrega lucro constante. Olha o que está sendo oferecido. Se existisse um programa que ganha dinheiro sozinho, todo dia, sem risco, o dono não venderia acesso por R$ 200. Ele rodaria o robô e ficaria de bico calado. Vender o robô só faz sentido se o dinheiro vier de quem compra o robô, e não do mercado.',
      'Consórcio como investimento: consórcio é um jeito de comprar uma coisa (carro, casa) sem juros de financiamento, pagando taxa de administração e esperando sorteio ou lance. É compra parcelada, não aplicação. Seu dinheiro fica preso por anos, não rende pra você, e dá pra pagar o consórcio inteiro sem nunca ser sorteado, esperando pelo bem até o fim do prazo. Chamar isso de investimento inverte o que está acontecendo: você não está juntando dinheiro, está pagando adiantado.',
      ...
      'A regra que resume tudo: rendimento alto e risco baixo não coexistem. Quando alguém junta os dois na mesma frase, a parte falsa é o risco baixo. O número alto é a isca, e o "sem risco" é a mentira que faz você morder.',
    ],
    goodFor: [
      'Entender por que o golpe precisa prometer demais: sem o exagero, ninguém entraria',
    ...
    watchOut: [
      ...
      'Entrar por indicação de alguém que "já ganhou". No começo sempre tem gente ganhando, e é isso que atrai a próxima leva.',
...
export const ORDER_MATTERS =
  ...
  'existe aplicação segura que renda mais rápido do que essa dívida cresce. Depois vêm as opções ' +
  'mais simples, aquelas de que o dinheiro sai rápido. Só no fim aparecem as que prendem o ' +
  'dinheiro ou que variam de preço no caminho. Quem começa pelo fim da lista costuma voltar pro ' +
  'começo dela, só que com menos dinheiro do que tinha.';
...
export type Scam = {
  id: string;
  /**
   * Papel do catálogo de `@/ui/Icon`. Mesma ressalva do `icon` de `Topic`:
   * NENHUMA TELA DESENHA ISTO. A lista de golpes não tem ícone de propósito,
   * porque seis linhas com o mesmo glifo de perigo não distinguem nada.
   */
  icon: IconName;
  title: string;
...
export const SCAMS: readonly Scam[] = [
  ...
  },
  {
    ...
    truth:
      'Clonar um perfil leva dez minutos e print de lucro se faz no editor de imagem. Repara no que está acontecendo: uma pessoa que ganha muito dinheiro no mercado não tem motivo nenhum pra te procurar no direct pra cuidar dos seus R$ 500. O produto é você. Assessor de verdade é registrado, atende vinculado a uma instituição, e dá pra conferir o registro dele no site da CVM, que é o órgão que fiscaliza o mercado, antes de mandar qualquer centavo.',
  },
  {
    id: 'renda-garantida',
    icon: 'danger',
...
export const GLOSSARY: readonly { term: string; plain: string }[] = [
  ...
  {
    term: 'rendimento',
    plain: 'O que o dinheiro ganhou por estar guardado, ou seja, o que apareceu além do que você colocou.',
  },
  {
    term: 'IR regressivo',
